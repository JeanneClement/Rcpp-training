...
* Rcpp
* RcppGSL require GSL installed (~$ sudo apt-get install libgsl-dev with Ubuntu)
* RcppArmadillo
* datasets
* rdist
* rbenchmark
...
## Data simulation 
** Simulation of 1000 points dispersed all over the space : **
\vspace{0.2cm}
...
$$\begin{aligned}
L(\beta,\sigma^2) &= \mathbb{P}(y_i \ | \ \beta, \sigma^2) \\
...
1. Implement a function in C++ using RcppArmadillo to compute the log-likelihood corresponding to the previous simple linear model taking as argument the explicative variables $X$, the response variable $Y$, the estimated regression coefficients $\widehat{\beta}$ and the calculated residual variance $\widehat{\sigma^2}$.
...
R_logL <- function(X,y,beta,sigma2) {
  ...
  p <- length(beta) # number of regression parameters 
...
We compare the rapidity of implemented functions in C++ and R to compute de log-likelihood corresponding of the model 3.
\vspace{0.2cm}
```{r logL-benchmark}
